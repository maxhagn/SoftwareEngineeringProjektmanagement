...
public class TestDataService {
    ...
    private void generatePerformances() {
        ...
        performanceRepository.saveAndFlush(Performance.builder().id(12L).event(event).hall(hall).datetime(LocalDateTime.now().minusDays(1)).build());
    ...
    private void generateFullEvent() {
        ...
        PriceCategory c3 = PriceCategory.builder().id(203L).name("Expensive").location(location).price(20.0f).build();
        c1 = priceCategoryRepository.saveAndFlush(c1);
        c2 = priceCategoryRepository.saveAndFlush(c2);
        c3 = priceCategoryRepository.saveAndFlush(c3);

        Hall h1 = Hall.builder().id(201L).cols(10).rows(25).name("Sall 1").location(location).build();
        h1 = hallRepository.saveAndFlush(h1);
        ...
        Area a3 = Area.builder().id(250L).startCol(2).endCol(9).startRow(21).endRow(25).hall(h1)
            .name("super").priceCategory(c3).type(Type.SECTION).build();
        screen = areaRepository.saveAndFlush(screen);
        a1 = areaRepository.saveAndFlush(a1);
        a2 = areaRepository.saveAndFlush(a2);
        a3 = areaRepository.saveAndFlush(a3);

        Performance p1 = Performance.builder().id(201L).event(event).hall(h1).datetime(LocalDateTime.now().plusDays(1)).build();
        Performance p2 = Performance.builder().id(202L).event(event).hall(h1).datetime(LocalDateTime.now().minusDays(1)).build();
        Performance p3 = Performance.builder().id(203L).event(event).hall(h1).datetime(LocalDateTime.now().minusDays(1)).build();
        p1 = performanceRepository.saveAndFlush(p1);
        p2 = performanceRepository.saveAndFlush(p2);
        p3 = performanceRepository.saveAndFlush(p3);
        ...
        Ticket ticket4 =
            Ticket.builder().id(204L).performance(p2).status(Status.IN_QUE).user(user).createDate(LocalDateTime.now()).build();
        ticketRepository.saveAndFlush(ticket4);

        Ticket ticket5 =
            Ticket.builder().id(205L).performance(p1).status(Status.IN_QUE).user(user).createDate(LocalDateTime.now()).build();
        ticketRepository.saveAndFlush(ticket5);

        Seat s17 = Seat.builder().id(217L).seatCol(1).seatRow(22).ticket(ticket5).area(a3).build();
        seatRepository.saveAndFlush(s17);

        Seat s15 = Seat.builder().id(215L).seatCol(1).seatRow(12).ticket(ticket3).area(a2).build();
        seatRepository.saveAndFlush(s15);

        Seat s16 = Seat.builder().id(216L).seatCol(2).seatRow(12).ticket(ticket4).area(a2).build();
        seatRepository.saveAndFlush(s16);
